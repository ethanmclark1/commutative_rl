...
generate_job_name() {
    ...
    local n_bridges=$8
    local n_holes=$9
    local n_episode_steps=${10}
    local configs_to_consider=${11}
    local action_success_rate=${12}
    local alpha=${13}
    local epsilon=${14}
    local gamma=${15}
    local batch_size=${16}
    local hidden_dims=${17}
    local buffer_size=${18}
    local target_update_freq=${19}
    local dropout=${20}
    echo "${approach}_${seed}_${problem_instance}_${grid_dims}_${noise_type}_${n_starts}_${n_goals}_${n_bridges}_${n_holes}_${n_episode_steps}_${configs_to_consider}_${action_success_rate}_${alpha}_${epsilon}_${gamma}_${batch_size}_${hidden_dims}_${buffer_size}_${target_update_freq}_${dropout}.sh"
}
                                                                    ...
                                                                    for buffer_size in "${BUFFER_SIZE[@]}"; do
                                                                        for target_update_freq in "${TARGET_UPDATE_FREQ[@]}"; do
                                                                            for dropout in "${DROPOUT[@]}"; do
                                                                                args="--approaches $approach --seed $seed --problem_instances instance_$problem_instance --grid_dims $grid_dims --noise_type $noise_type --n_starts $n_starts --n_goals $n_goals --n_bridges $n_bridges --n_holes $n_holes --n_episode_steps $n_episode_steps --configs_to_consider $configs_to_consider --action_success_rate $action_success_rate --alpha $alpha --epsilon $epsilon --gamma $gamma --batch_size $batch_size --hidden_dims $hidden_dims --buffer_size $buffer_size --target_update_freq $target_update_freq --dropout $dropout"
                                                                                job_name=$(generate_job_name "$approach" "$seed" "$problem_instance" "$grid_dims" "$noise_type" "$n_starts" "$n_goals" "$n_bridges" "$n_holes" "$n_episode_steps" "$configs_to_consider" "$action_success_rate" "$alpha" "$epsilon" "$gamma" "$batch_size""$hidden_dims" "$buffer_size" "$target_update_freq" "$dropout")
