...
class RewardBuffer:
    ...
    def add(
        ...
        self.transition[self.count] = torch.cat(
            [state, action_idx, next_state, num_action]
        )
        self.reward[self.count] = torch.as_tensor([reward])
        self.is_initialized[self.count] = True
...
class CommutativeRewardBuffer(RewardBuffer):
    ...
    def add(
        ...
        step_0 = torch.cat([prev_state, action_idx, commutative_state, prev_num_action])

        prev_action_idx = torch.as_tensor(prev_action_idx)
        next_state = torch.as_tensor(next_state)
        num_action = torch.as_tensor(num_action)
        step_1 = torch.cat([commutative_state, prev_action_idx, next_state, num_action])
...
def encode(
    input_value: Union[List, torch.Tensor], dims: int
