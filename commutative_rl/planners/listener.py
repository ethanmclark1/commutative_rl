...
class Listener:
    def __init__(self, agent_radius, obstacle_radius):
        ...
        self.inflation_radius = int(
            round((agent_radius + obstacle_radius) / self.resolution)
        )
    ...
    def _clean_graph(self, graph, obstacle_nodes, agent_node, target_node):
        ...
        for obstacle_node in obstacle_nodes:
            for dx, dy in itertools.product(
                range(-self.inflation_radius, self.inflation_radius + 1), repeat=2
            ):
                ...
                if dx**2 + dy**2 <= self.inflation_radius**2:
                    ...
                    if (
                        0 <= inflated_node[0] < self.grid_conversion_factor
                        and 0 <= inflated_node[1] < self.grid_conversion_factor
                    ):
    ...
    def get_action(self, observation, directions, approach, language):
        ...
        target_pos = self._get_target(
            agent_pos, goal_pos, directions, approach, language
        )
        ...
        agent_node = tuple(
            map(round, ((np.array(agent_pos) + 1) * self.grid_conversion_factor / 2))
        )
        target_node = tuple(
            map(round, ((np.array(target_pos) + 1) * self.grid_conversion_factor / 2))
        )
        obstacle_nodes = set(
            tuple(
                map(round, ((np.array(obstacle) + 1) * self.grid_conversion_factor / 2))
            )
        ...
        try:
            path = nx.astar_path(
                graph, agent_node, target_node, heuristic=euclidean_dist
            )

            next_state = np.array(path[1]) * self.resolution - 1
            direction = (next_state - agent_pos) / np.linalg.norm(
