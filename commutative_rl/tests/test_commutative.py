...
class TestCommutative(unittest.TestCase):
    def __init__(self, methodName: str = 'runTest', **kwargs):
        ...
        self.env = Env(self.seed, self.num_instances, self.max_sum, self.action_dims, self.reward_type, self.reward_noise)
    ...
    def test_commutative(self, num_tests: int = 25):
        ...
        for num_instance in range(self.num_instances):
            ...
            for action_set in action_sets:
                final_state = action_set + [0] * (self.env.max_elements - len(action_set))
                ...
                for perm in permutations:
                    ...
                    self.assertEqual(original_state, perm_state, 
                                    f"States differ for permutation {perm}")
                    self.assertAlmostEqual(original_return, perm_return, places=5,
                                        msg=f"Returns differ for permutation {perm}")
                
                print(f"Commutativity test passed for final state: {original_state} with final return: {original_return}")
...
if __name__ == '__main__':
    run_tests(seed=42, num_instances=30, max_sum=100, action_dims=30, reward_type='true', reward_noise=0.0)